...
impl Game {
    ...
    fn add_food(&mut self) {
        ...
        let mut new_x = rng.gen_range(2..=self.width - 2);
        let mut new_y = rng.gen_range(2..=self.height - 2);

        while self.snake.overlap_tail(new_x, new_x) {
    ...
    fn restart(&mut self) {
        ...
        while self.snake.overlap_tail(new_food_x, new_food_x) {
            new_food_x = rng.gen_range(2..=self.width - 2);
            new_food_y = rng.gen_range(2..=self.height - 2);
        };
